...
const JobDescription = () => {
  ...
  const applyJobHandler = async () => {
    try {
      ...
      const res = await axios.get(`https://job-portal-app-irvt.onrender.com/api/v1/application/apply/${params.id}`);
      if (res.data.success) {
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        axios.defaults.withCredentials = true;
        const res = await axios.get(`https://job-portal-app-irvt.onrender.com/api/v1/job/${params.id}`);
        if (res.data.success) {
    ...
    <div className='max-w-7xl mx-auto my-10'>
      <div className='flex items-center justify-between'>
        ...
        <Button
          ...
          disabled={isApplied}
          className={`rounded-lg ${isApplied ? "bg-gray-600 cursor-not-allowed" : "bg-[#7209b7] hover:bg-[#da199d]"}`}
        >
